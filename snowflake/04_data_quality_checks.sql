...
SELECT 'Ship before order date' AS check_name,
       COUNT(*) AS row_count
FROM RAW.raw_orders
WHERE TRY_TO_DATE(ship_date, 'MM/DD/YYYY') < TRY_TO_DATE(order_date, 'MM/DD/YYYY');
...
WHERE DATEDIFF('day',
    ...
    TRY_TO_DATE(ship_date,  'MM/DD/YYYY')) > 30;
...
SELECT
    ...
    DATEDIFF('day',
        MIN(TRY_TO_DATE(order_date, 'MM/DD/YYYY')),
        MAX(TRY_TO_DATE(order_date, 'MM/DD/YYYY'))
    )                                           AS date_span_days
FROM RAW.raw_orders;
...
WHERE order_date IS NOT NULL
  AND TRY_TO_DATE(order_date, 'MM/DD/YYYY') IS NULL;

SELECT 'Unparseable ship_date' AS check_name,
       COUNT(*) AS row_count
FROM RAW.raw_orders
WHERE ship_date IS NOT NULL
  AND TRY_TO_DATE(ship_date, 'MM/DD/YYYY') IS NULL;
...
SELECT check_name, result, status FROM (
    ...
    SELECT 'Ship before order',                       COUNT(*),           IFF(COUNT(*)=0,'PASS','FAIL')            FROM RAW.raw_orders WHERE TRY_TO_DATE(ship_date,'MM/DD/YYYY') < TRY_TO_DATE(order_date,'MM/DD/YYYY') UNION ALL
    SELECT 'Unparseable order_date',                  COUNT(*),           IFF(COUNT(*)=0,'PASS','FAIL')            FROM RAW.raw_orders WHERE order_date IS NOT NULL AND TRY_TO_DATE(order_date,'MM/DD/YYYY') IS NULL
)
